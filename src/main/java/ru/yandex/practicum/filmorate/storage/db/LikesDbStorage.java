...
public class LikesDbStorage implements LikesStorage {
    ...
    public List<Film> getFilms(int limit) {
        List<Film> films = jdbcTemplate.query("select f.film_id, r.rate, f.name, f.description, f.releaseDate, " +
                "f.duration, m.mpa_id, m.name as mpa_name from (select l.film_id as film_id, " +
                "count(l.user_id) as rate from likes l group by l.film_id) as r " +
                "right join films as f on r.film_id = f.film_id " +
                "join mpa m on f.mpa = m.mpa_id  order by r.rate desc limit ?", FilmDbStorage::createFilm, limit);
